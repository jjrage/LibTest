...
using AudioSettings = StreamingLibrary.AudioSettings;
...
public class StreamingTest : MonoBehaviour
{
    ...
    private void StartStream()
    {
        if(!string.IsNullOrEmpty(_userNameInput.text) && !string.IsNullOrEmpty(_channelNameInput.text))
        {
            ...
            streamingServiceSettings.LocalCamera = Camera.main; //Camera from local scene that capturing video data into render texture.
            ...
            AudioSettings audioSettings = new AudioSettings(_audioBitrate); //Prefered aduio settings.
